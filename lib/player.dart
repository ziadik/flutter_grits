...
class _MapLoaderScreenState extends State<MapLoaderScreen> {
  ...
  Future<void> _loadResources() async {
    try {
      ...
      // Загружаем эффекты
      final effectsJson = await rootBundle.loadString(
...
class _EffectsMapScreenState extends State<EffectsMapScreen>
  ...
  void initState() {
    ...
    // Устанавливаем начальную позицию игрока в центре карты (в тайловых координатах)
  ...
  void _fitMapToScreen() {
    ...
    final mapPixelWidth = _mapWidth * _tileWidth;
    final mapPixelHeight = _mapHeight * _tileHeight;
  ...
  void _handleInput() {
    ...
    if (dx != 0 || dy != 0) {
      setState(() {
        ...
        // Границы карты (с учетом размера игрока)
        final minX = 0.0;
        final maxX =
            (_mapWidth * _tileWidth).toDouble() -
            64; // 64 - половина ширины игрока
        final minY = 0.0;
        final maxY = (_mapHeight * _tileHeight).toDouble() - 64;
  ...
  void _updateCameraPosition() {
    ...
    // Добавляем половину размера игрока для правильного центрирования
    final targetX = (_playerX + 64) * scale - size.width / 2;
    final targetY = (_playerY + 64) * scale - size.height / 2;
  ...
  void _centerCameraOnPlayer() {
    ...
    final x = (_playerX + 64) * scale - size.width / 2;
    final y = (_playerY + 64) * scale - size.height / 2;
        ...
        appBar: AppBar(
          ...
          actions: [
            ...
            IconButton(
              ...
              tooltip: _followPlayer
                  ? 'Отключить слежение'
                  : 'Слежение за игроком',
            ),
            IconButton(
              icon: Icon(
                _showEffects ? Icons.visibility : Icons.visibility_off,
              ),
          ...
          child: Stack(
            children: [
              SizedBox(
                ...
                child: TileMapViewerWithEffects(
              ...
              if (_showPlayer && _effectsImageInfo != null)
                Positioned(
                  ...
                  child: GamePlayerWidget(
                    ...
                    playerX:
                        0, // Передаем 0, так как Positioned уже задает позицию
                    playerY: 0,
...
class CameraController {
  ...
  final double _lerpFactor =
      0.1; // Коэффициент интерполяции (чем меньше, тем плавнее)
  ...
  CameraController({
    ...
    int mapWidth = 64,
    int mapHeight = 48,
    double tileWidth = 64.0,
    double tileHeight = 64.0,
  ...
  void moveCamera(double dx, double dy) {
    if (_enableBounds) {
      ...
      _targetY = (_targetY - dy).clamp(
        0,
        _mapHeight * _tileHeight - _tileHeight,
      );
    } else {
      _targetX -= dx;
      _targetY -= dy;
...
class TileMapViewerWithEffects extends StatefulWidget {
  ...
  @override
  _TileMapViewerWithEffectsState createState() =>
...
class _TileMapViewerWithEffectsState extends State<TileMapViewerWithEffects> {
  ...
  Future<void> _loadResources() async {
    try {
      ...
      final [tileInfo, effectsInfo] = await Future.wait([
...
class SpriteSheet {
  ...
  factory SpriteSheet.fromJson(
    ...
    return SpriteSheet(
      ...
      sourceSize: Size(
        (metaSize['w'] as num).toDouble(),
        (metaSize['h'] as num).toDouble(),
      ),
    );
  }
  ...
  SpriteData? findSprite(String name) {
    ...
    final matches = sprites.keys.where((key) {
      ...
      return keyWithoutExt.contains(normalizedKey) ||
          normalizedKey.contains(keyWithoutExt);
    }).toList();
